...
[
  ...
  AS_IF([test "$sst_check_spatter_happy" = "yes"], [
    AS_IF([test ! -z "$with_spatter" -a "$with_spatter" != "yes"],
      [SPATTER_CPPFLAGS="-I$with_spatter/../src -I$with_spatter/_deps/nlohmann_json-src/include"
  ...
  AC_CHECK_HEADERS([nlohmann/json.hpp \
                    Spatter/Configuration.hh \
                    Spatter/Input.hh \
                    Spatter/JSONParser.hh \
                    Spatter/PatternParser.hh \
                    Spatter/SpatterTypes.hh \
                    Spatter/Timer.hh], [], [sst_check_spatter_happy="no"])
  AC_LANG_POP([C++])
